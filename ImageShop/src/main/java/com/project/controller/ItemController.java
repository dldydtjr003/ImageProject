...
public class ItemController {
	...
	public String buy(Item item, RedirectAttributes rttr, Authentication authentication) throws Exception {
		...
		if (count != 0) {
			...
			rttr.addFlashAttribute("msg", "구매가 완료 되었습니다.");
		} else {
			rttr.addFlashAttribute("msg", "구매가 실패 되었습니다.");
		}
	...
	public ResponseEntity<byte[]> displayFile(Item item) throws Exception {
		...
		try {
			...
			// httpHeader 서버가 브라우저에게 경고를 담아서 보내주는 객체
			HttpHeaders headers = new HttpHeaders();

			// 파일을 inputStream(바이트 단위로 읽는다.)
			// D:/upload "/" "fileName" 위치를 찾아서 파일을 inputStream으로 읽는다.
			in = new FileInputStream(uploadPath + File.separator + fileName);

			if (mType != null) {
				// httpHeader 서버가 브라우저에게 경고를 담아서 보내주는 객체에 미디어 타입을 셋팅
